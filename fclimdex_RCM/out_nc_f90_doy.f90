...
!... define the variables used in this program !
    ...
    integer LATID, LONID, TIMEID,DOYID                ! dimention's ID
    integer varID_lon,varID_lat,varID_time,varID_doy      ! variable's ID
...
call err_handle(NF90_def_var(ncid, 'lon', NF90_float, coordim,varID_lon),'define var lon')
call err_handle(NF90_def_var(ncid, 'lat', NF90_float, coordim,varID_lat),'define var lat')
call err_handle(NF90_def_var(ncid, 'doy', NF90_int, doyID,varID_doy),'define var doy')
else
...
print *,datadim
print *,ID_data(i)
call err_handle(NF90_def_var(ncid, trim(ctit(i)), NF90_float, datadim, ID_data(i)),'temp define data')
call err_handle(NF90_put_att(ncid, ID_data(i), 'missing_value', MISSING),'put att for data')
call err_handle(NF90_put_att(ncid, ID_data(i), '_FillValue', MISSING),'put att for data')
enddo

do i=1,2
call err_handle(NF90_def_var(ncid, trim(ctip(i)), NF90_float, datadimpr, ID_datapr(i)),'define prec data')
...
call err_handle(NF90_put_att(ncid, varID_doy, 'units', 'Day of the year - 365'),'put att for time')
